...
from backend_ide.ui.theme.tokens import DARK_PALETTE, LIGHT_PALETTE
...
def test_theme_manager_persists_all_documented_modes(tmp_path):
    """Ensure ThemeManager saves and restores System, Light, and Dark modes."""
...
def test_system_theme_resolves_to_a_concrete_palette(tmp_path):
    """System theme must resolve to either LIGHT or DARK palette."""
...
def test_theme_control_exposes_system_light_and_dark(app_instance, qtbot):
    """Theme toggle button should expose 3 modes in menu and allow toggling."""
...
def test_top_bar_groups_controls_by_documented_function(app_instance, qtbot):
    """Verify 3-column top bar structure, dimensions, and action buttons."""
    app, window = app_instance
    qtbot.addWidget(window)
    window.show()
    app.processEvents()
    ...
    assert window.btn_er_diagram.isEnabled()


def test_connection_controls_follow_context_then_actions(app_instance, qtbot):
    """Verify connection selector layout order: profile -> combo -> badge -> new -> edit."""
    _, window = app_instance
    qtbot.addWidget(window)
...
def test_header_rows_stay_compact_and_controls_are_vertically_aligned(app_instance, qtbot):
    ...
    assert max(vertical_centers) - min(vertical_centers) <= 4
...
def test_documented_window_sizes_have_no_clipped_toolbar_controls(app_instance, qtbot, size):
    """Toolbar controls must not clip at both initial and minimum documented sizes."""
    app, window = app_instance
    qtbot.addWidget(window)
    window.resize(*size)
    window.show()
